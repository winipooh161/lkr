...
function showLoading(show) {
    // Сначала пытаемся использовать существующий loadingIndicator
    const loadingIndicator = document.getElementById('loadingIndicator');
    if (loadingIndicator) {
        if (show) {
            loadingIndicator.style.display = 'flex';
            loadingIndicator.innerHTML = '<div class="spinner-border text-primary" role="status"></div><div class="mt-2">Сохранение...</div>';
        } else {
    ...
    if (show) {
        // Создаем и показываем индикатор загрузки, если нет основного
        if (!loadingIndicator) {
...
/**
 * Принудительная очистка всех индикаторов загрузки
 */
