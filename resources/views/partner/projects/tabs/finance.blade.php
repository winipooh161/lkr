        ...
        <div class="table-responsive-mobile">
            <table class="table table-borderless">
                <tbody>
                    ...
                    <tr>
                        <th>Общая сумма:</th>
                        <td>{{ number_format($project->work_amount + $project->materials_amount, 2, '.', ' ') }} ₽</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<div class="modal fade" id="editItemModal" tabindex="-1" aria-labelledby="editItemModalLabel" aria-hidden="true" style="z-index: 1060 !important;">
    <div class="modal-dialog" style="z-index: 1061 !important;">
        <div class="modal-content" style="z-index: 1061 !important;">
            <div class="modal-header" style="z-index: 1062 !important;">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('saveAddItemBtn').addEventListener('click', function() {
            ...
            fetch('{{ route('partner.projects.finance.store', $project) }}', {
                method: 'POST',
                headers: {
        ...
        document.getElementById('saveEditItemBtn').addEventListener('click', function() {
            ...
            // Отправка данных на сервер
            fetch(`{{ url('partner/projects-finance') }}/${itemId}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
        ...
        function loadFinanceData() {
            return fetch('{{ route('partner.projects.finance.index', $project) }}')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
