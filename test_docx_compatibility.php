...
echo "Тестирование совместимости HTML с PhpWord...\n\n";
...
// Тест 1: Оригинальный HTML
echo "=== ТЕСТ 1: Оригинальный HTML ===\n";
try {
    $phpWord = new PhpWord();
    $section = $phpWord->addSection();
    
    \PhpOffice\PhpWord\Shared\Html::addHtml($section, $testHtml);
    
    $tempFile = tempnam(sys_get_temp_dir(), 'docx_test1_');
    $objWriter = IOFactory::createWriter($phpWord, 'Word2007');
    $objWriter->save($tempFile);
    
    echo "✅ УСПЕХ: Оригинальный HTML успешно обработан\n";
    echo "📄 Файл сохранен: $tempFile\n\n";
    
} catch (Exception $e) {
    echo "❌ ОШИБКА: " . $e->getMessage() . "\n\n";
}

// Тест 2: Минимальная очистка
echo "=== ТЕСТ 2: Минимальная очистка HTML ===\n";
try {
    ...
    $tempFile = tempnam(sys_get_temp_dir(), 'docx_test2_');
    $objWriter = IOFactory::createWriter($phpWord, 'Word2007');
    $objWriter->save($tempFile);
...
// Тест 3: Оптимизированный HTML
echo "=== ТЕСТ 3: Оптимизированный HTML ===\n";
try {
    ...
    \PhpOffice\PhpWord\Shared\Html::addHtml($section, $optimizedHtml);
    
    $tempFile = tempnam(sys_get_temp_dir(), 'docx_test3_');
    $objWriter = IOFactory::createWriter($phpWord, 'Word2007');
    $objWriter->save($tempFile);
...
// Тест 4: Простейший HTML без стилей
echo "=== ТЕСТ 4: Простейший HTML ===\n";
...
try {
    ...
    \PhpOffice\PhpWord\Shared\Html::addHtml($section, $simpleHtml);
    
    $tempFile = tempnam(sys_get_temp_dir(), 'docx_test4_');
    $objWriter = IOFactory::createWriter($phpWord, 'Word2007');
    $objWriter->save($tempFile);
